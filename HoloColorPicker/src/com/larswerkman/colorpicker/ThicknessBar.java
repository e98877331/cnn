 ...
 * 
 * modified by cheng-yen Yang 2013
 */
...
public class ThicknessBar extends View {
	...
	private float mPosToThickFactor;
	private float mThickToPosFactor;
	...
	public ThicknessBar(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs, 0);
	}

	public ThicknessBar(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		mPosToThickFactor= 100 / ((float) mBarLength);
		mThickToPosFactor = ((float) mBarLength)/ 100;
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (x >= (mBarPointerHaloRadius)
				...
				calculateThickness(Math.round(x));
		...
		case MotionEvent.ACTION_MOVE:
			if (mIsMovingPointer) {
				...
				if (x >= mBarPointerHaloRadius
					...
					if (mPicker != null) {
						mPicker.setNewWheelThickness(mThickness);
					}
					invalidate();
...
//		}
	...
	/**
	 * Set the pointer on the bar. With the thickness value.
	 * 
	 * @param saturation
	 *            float between 0 > 255
	 */
...
//		mBarPointerPosition = Math.round((mOpacToPosFactor * pThickness))
//				+ mBarPointerHaloRadius;
		mBarPointerPosition =  Math.round((mThickToPosFactor * pThickness))+ mBarPointerHaloRadius;
		Log.e("test",Integer.toString(mBarPointerPosition));
		//calculateThickness(mBarPointerPosition);
		//mBarPointerPaint.setColor(mColor);
		if (mPicker != null) {
			//mPicker.setNewCenterColor(mColor);
		}
	 ...
	 */
        private void calculateThickness(int x) {
    		...
    		//mThickness = Math.round(mPosToOpacFactor * x /2 +10);
    			mThickness = Math.round(mPosToThickFactor * x);
